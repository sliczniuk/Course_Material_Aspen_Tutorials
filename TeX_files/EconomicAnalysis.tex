...
%	\end{figure}
	...
	It is not always possible to simulate a whole production process, in particular in case of bio-based processes. The APEA can be used to estimate an equipment price, based on limited amount of informations. To use the APEA without the process simulation, piece of equipment from the \textit{Process equipment} section. \textit{Process equipment} section is usually located in the top-right corner in the \textit{Components} tab. Then, an analysed equipment needs to be added in the \textit{list} section by drag-and-drop. The selection of an equipment from \textit{Process equipment} section corresponds to the mapping procedure, which was discussed before. The drag-and-drop will cause the opening of a new windows called \textit{New Component information}, where the name of analysed machine needs to be assign. After acceptance of the equipment name, a new windows with technical sheet will open. Depending on the equipment, different information about the equipment size are required to evaluate the cost. The necessary information are highlighted. In case of a vessel, a necessary information is it radius and height. If a heat exchanger is discussed, then a heat transfer area needs to be know a priori. 
